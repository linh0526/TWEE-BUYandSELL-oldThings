...
export default function MyOrdersScreen() {
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      if (newStatus === 'paid') {
        const { data: orderData } = await supabase.from('orders').select('product_id, quantity').eq('id', orderId).single();
        if (orderData) {
          const { data: prod } = await supabase.from('products').select('quantity').eq('id', orderData.product_id).single();
          if (prod) {
            const newStock = Math.max(0, (prod.quantity || 0) - (orderData.quantity || 0));
            await supabase.from('products').update({ quantity: newStock }).eq('id', orderData.product_id);
          }
        }
      }
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={['top']}>
      ...
      {type === 'selling' && (
        <View className="flex-row px-6 py-3 bg-gray-50/50">
          <TouchableOpacity 
          ...
          >
            <Text className={`font-black text-[10px] uppercase ${activeSubTab === 'products' ? 'text-white' : 'text-gray-400'}`}>Tin đang bán</Text>
          </TouchableOpacity>
          <TouchableOpacity 
      ...
      {loading ? (
        ...
        <ScrollView className="flex-1 bg-gray-50/30" showsVerticalScrollIndicator={false}>
                    ...
                    <View>
                      ...
                      <View className="flex-row items-center mt-1">
                        <View className="bg-gray-100 px-2 py-0.5 rounded-md mr-2">
                          <Text className="text-[8px] font-black text-gray-400 uppercase">Kho: {item.stock || 0}</Text>
                        </View>
                        <Text className="text-[8px] font-black text-secondary uppercase italic">{item.condition || 'Tốt'}</Text>
                      </View>
                    </View>
          ...
          {(activeSubTab === 'orders' ? orders.length : products.length) === 0 && (
